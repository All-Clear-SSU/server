...
public class WifiDetectionService {
     ...
     *
     * @param sensorCode WiFi 센서 코드 (예: "ESP32-001")
     * @param limit 조회할 최대 개수 (기본값: 50, 최대값: 200)
     * @return WiFi 탐지 기록 목록 (최신순)
     */
    public List<WifiDetectionRecordDto> getRecentDetections(String sensorCode, Integer limit) {
        log.info("WiFi 센서 최근 탐지 기록 조회 요청 - 센서: {}, limit: {}", sensorCode, limit);

        // limit 파라미터 검증 및 기본값 설정을 수행함
        int validatedLimit = validateAndSetLimit(limit);

        // WiFi 센서를 조회함
        WifiSensor sensor = wifiSensorRepository.findBySensorCode(sensorCode)
                .orElseThrow(() -> new IllegalArgumentException("WiFi 센서를 찾을 수 없습니다. 센서 코드: " + sensorCode));
        ...
        log.info("WiFi 센서 탐지 기록 조회 완료 - 센서: {}, 조회된 개수: {}", sensorCode, detections.size());
